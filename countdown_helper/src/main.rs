...
fn main() {
    ...
    // Solver::solve(100, vec!(1,2,3,4,20,70));
    // Solver::solve(808, vec!(75, 100, 4, 1, 3, 3));
    Solver::solve(768, vec!(100, 7, 2, 9, 8, 4));
    // Solver::solve(24, vec!(8, 4, 3, 3));
...
impl Number {
    ...
    fn print(&self) -> String {
        match self {
            ...
            &Difference(ref value, ref a, ref b) => format!("[{} - {} = {}]", a.print(), b.print(), value),
            &Product(ref value, ref a, ref b) => format!("[{} * {} = {}]", a.print(), b.print(), value),
            &Quotient(ref value, ref a, ref b) => format!("[{} / {} = {}]", a.print(), b.print(), value),
        }
    }
    ...
    fn num_in_history(&self, num: &usize) -> bool {
        match self {
            &Atomic(ref value) => value == num,
            &Sum(ref value, ref a, ref b) |
            &Difference(ref value, ref a, ref b) |
            &Product(ref value, ref a, ref b) |
            &Quotient(ref value, ref a, ref b) => value == num || a.num_in_history(num) || b.num_in_history(num),
        }
    }
    ...
    fn new_difference(a: Rc<Number>, b: Rc<Number>) -> Option<Rc<Number>> {
        if Number::is_valid_difference(&a, &b)
                && Number::is_sensible(&(a.value() - b.value()), &a, &b) {
    ...
    fn new_quotient(a: Rc<Number>, b: Rc<Number>) -> Option<Rc<Number>> {
        if Number::is_valid_quotient(&a, &b)
                && Number::is_sensible(&(a.value() / b.value()), &a, &b) {
...
impl PartialEq for Number {
    fn eq(&self, other: &Number) -> bool {
        match (self, other) {
            ...
            (&Difference(_, ref a1, ref a2), &Difference(_, ref b1, ref b2)) => non_commutative(a1, a2, b1, b2),
            (&Product(_, ref a1, ref a2), &Product(_, ref b1, ref b2)) => commutative(a1, a2, b1, b2),
            (&Quotient(_, ref a1, ref a2), &Quotient(_, ref b1, ref b2)) => non_commutative(a1, a2, b1, b2),
            _ => false,
        }
    }
}
...
impl Solver {
    ...
    fn new() -> Solver {
        Solver {solutions: Vec::new()}
    }

    fn solve(target: usize, numbers: Vec<usize>) {
        println!("Solver created for target {} and numbers {:?}", target, numbers);
        Solver::new().compute(target, Solver::prep(numbers));
    }
